...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  latLongToggles->addItem(new CValueControl("SteerRatioRate", "自定义转向比倍率SteerRatioRate ×0.01(100)", "实时转向比应用比例（需将 CustomSR 设为 0 才生效）", 30, 170, 1));
  latLongToggles->addItem(new CValueControl("PathOffset", "横向路径偏移", "(-)向左偏移, (+)向右偏移", -150, 150, 1));
  latLongToggles->addItem(new CValueControl("SteerActuatorDelay", "横向控制：转向执行延迟(30)", "x0.01, 转向延迟值， 数值越大，转向越早。设为 0 则使用实时延迟。", 0, 100, 1));
  latLongToggles->addItem(new CValueControl("LateralTorqueCustom", "横向控制：横向力矩自定义(0)", "启用自定义力矩（力矩加速度因子、摩擦系数）", 0, 2, 1));
  latLongToggles->addItem(new CValueControl("LateralTorqueAccelFactor", "横向控制：横向力矩加速度系数(2500)", "数值越小，力矩先逐渐增强到最强点，随后减弱——系统取该最强点对应的数值。", 1000, 6000, 10));
  latLongToggles->addItem(new CValueControl("LateralTorqueFriction", "横向控制：横向力矩摩擦系数(100)", "数值越高，转向越细腻；建议设定范围 0–50。", 0, 1000, 10));
  latLongToggles->addItem(new CValueControl("CustomSteerMax", "横向控制：自定义最大转向力矩(0)", "最大转向力矩值，请选用 270/384/409 中不会触发报错的最大数值", 0, 30000, 5));
  latLongToggles->addItem(new CValueControl("CustomSteerDeltaUp", "横向控制：自定义转向力矩递增上限(0)", "扭矩车辆端 — 数值越高，方向盘转动速度越快", 0, 50, 1));
  latLongToggles->addItem(new CValueControl("CustomSteerDeltaDown", "横向控制：自定义转向力矩递减下限(0)", "扭矩车辆端 — 数值越大，方向盘回正速度越快", 0, 50, 1));
  ...
  latLongToggles->addItem(new CValueControl("LongActuatorDelay", "纵向执行延迟（LongActuatorDelay）(20)", "纵向延迟值，数值越大，加速/减速动作越提前，HKG 车型建议设为 20", 0, 200, 5));
  latLongToggles->addItem(new CValueControl("VEgoStopping", "起步车速阈值（VEgoStopping）(50)", "数值越小，起步越迅速；若设置过低，碰撞风险上升。建议设为 5", 1, 100, 5));
  latLongToggles->addItem(new CValueControl("RadarReactionFactor", "纵向控制：雷达反应系数(100)", "数值越低，对前车的响应越快", 0, 200, 10));
  latLongToggles->addItem(new CValueControl("StoppingAccel", "纵向控制：起步加速度x0.01(-40)", "车辆由静止启动时的加速度；设为 0 时采用传统计算方式", -100, 0, 5));
  latLongToggles->addItem(new CValueControl("StopDistanceCarrot", "纵向控制：停止距离 (600)cm", "检测到红绿灯时的停车位置", 300, 1000, 10));
  latLongToggles->addItem(new CValueControl("JLeadFactor3", "纵向控制：前车加速度响应系数(0)", "x0.01，数值越高，系统对前车加速度变化的响应越敏感", 0, 100, 5));
